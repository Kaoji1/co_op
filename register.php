...
<body>
  ...
  if (isset($_POST['submit_form'])) {
    ...
    if (isset($_FILES['Std_picture']) && $_FILES['Std_picture']['error'] === UPLOAD_ERR_OK) {
      ...
      $fileSize = $_FILES['Std_picture']['size'];
      $fileType = $_FILES['Std_picture']['type'];
      $fileExtension = strtolower(pathinfo($fileName, PATHINFO_EXTENSION));

      // กำหนดชื่อไฟล์ใหม่เพื่อป้องกันชื่อซ้ำ
      $newFileName = "profile_" . $_POST['studentId'] . "." . $fileExtension;
      $uploadFileDir = 'profile_pic/'; // โฟลเดอร์เก็บรูป
      $dest_path = $uploadFileDir . $newFileName; // แก้ไขให้รวม path
      ...
      if (in_array($fileExtension, $allowedFileExtensions)) {
        ...
        // อัปโหลดไฟล์ไปยังโฟลเดอร์ profile_pic
        if (move_uploaded_file($fileTmpPath, $dest_path)) {
          $std_picture = $newFileName; // บันทึกเฉพาะชื่อไฟล์ลงฐานข้อมูล
        } else {
          echo "<script>alert('เกิดข้อผิดพลาดในการอัปโหลดไฟล์');</script>";
          $std_picture = ''; // หากอัปโหลดไม่สำเร็จ ให้เก็บค่าเป็นค่าว่าง
        }
      } else {
        echo "<script>alert('ประเภทไฟล์ไม่ถูกต้อง อัปโหลดได้เฉพาะ JPG, PNG, GIF');</script>";
        $std_picture = '';
      }
    } else {
      $std_picture = ''; // ถ้าไม่มีการอัปโหลดไฟล์
    }

    // ดึง Major_id และ Tec_id จากสาขาที่เลือก
    $major_name = mysqli_real_escape_string($conn, $_POST['branch']);
    $sql_major = "SELECT m.Major_id, t.Tec_id, t.Tec_name 
              FROM major m
  ...
  <script>
    function validateForm() {
      let isValid = true;
      let inputs = document.querySelectorAll('#section1 input, #section1 select');

      inputs.forEach(input => {
        if (input.value.trim() === '') {
          input.style.border = '2px solid red'; // ทำให้เห็นว่าไม่ได้กรอก
          isValid = false;
        } else {
          input.style.border = ''; // ล้างเส้นแดงถ้ากรอกแล้ว
        }
      });
